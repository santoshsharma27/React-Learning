...
function UserProfileCard({
  user: { name, email, avatar, role, status, stats },
  theme: { backgroundColor, textColor, avatarBg, badgeBg },
  actions,
}) {
  return (
    <div className={`p-6 rounded-xl shadow-lg ${backgroundColor} ${textColor}`}>
      <div className="flex items-start gap-4">
        <div
          className={`w-16 h-16 rounded-full ${avatarBg} flex items-center justify-center text-2xl`}
        >
          {avatar}
        </div>
        <div className="flex-1">
          ...
          <div className="flex gap-2 text-sm">
            <span className={`px-3 py-1 rounded-full ${badgeBg}`}>{role}</span>
            <span className={`px-3 py-1 rounded-full ${badgeBg}`}>
              {status}
            </span>
          </div>
        </div>
